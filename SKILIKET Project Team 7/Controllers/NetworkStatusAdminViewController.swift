...
class networkStatusAdminViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet weak var managementIpAddressLabel: UILabel!  // Nuevo: para mostrar la IP de gestión
    @IBOutlet weak var deviceIdLabel: UILabel!  // Nuevo: para mostrar el ID del router
    ...
    @IBOutlet weak var selectedDeviceHostIpLabel: UILabel!  // Nuevo: para mostrar la IP del host
    @IBOutlet weak var selectedDeviceIdLabel: UILabel!  // Nuevo: para mostrar el ID del host
    ...
    func updateUI(with networkDevices: [NetworkDeviceResponse]) {
        DispatchQueue.main.async {
            if let firstDevice = networkDevices.first {
                ...
                // Nuevo: Mostrar la IP de gestión y el ID del router
                self.managementIpAddressLabel.text = "IP de Gestión: \(firstDevice.managementIpAddress)"
                self.deviceIdLabel.text = "ID del Router: \(firstDevice.id)"
            } else {
    ...
    func updateDeviceUI(with selectedDevice: Host) {
        DispatchQueue.main.async {
            ...
            // Nuevo: Mostrar la IP del host y el ID del host
            self.selectedDeviceHostIpLabel.text = "IP del Host: \(selectedDevice.hostIp)"
            self.selectedDeviceIdLabel.text = "ID del Host: \(selectedDevice.id)"
        }
    }
}
